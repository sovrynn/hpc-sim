...
def main():
    ...
    if not frame_to_degrees:
        ...
        last_specified_frame = None
        last_degrees = 0.0
    else:
        # Find the last specified frame and its rotation
        last_specified_frame = max(frame_to_degrees.keys())
        last_degrees = frame_to_degrees[last_specified_frame]
    ...
    for idx, filename in enumerate(png_files, start=1):
        ...
        if frame_number in frame_to_degrees:
            ...
            if last_specified_frame is not None and frame_number > last_specified_frame:
                # After the last specified frame: reuse last rotation value
                degrees = last_degrees
            else:
                # Before any specified frame, or gaps before the last specified frame
                degrees = 0.0
